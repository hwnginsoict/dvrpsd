                ...
                for route in self.routes:
                    for i in range(1, len(route)):
                        ...
                        if self.check_capacity([new_route], self.max_capacity) and self.check_time([new_route]):
                            increase = (self.network.links[(route[i - 1].node, request.node)].distance +
                                        self.network.links[(request.node, route[i].node)].distance -
                                        self.network.links[(route[i - 1].node, route[i].node)].distance)
                ...
                if solution_time + self.network.links[(current_node.node, request.node)].distance < request.end:
                    if solution_time + self.network.links[(current_node.node, request.node)].distance >= request.start:
                        total += (self.pheromone[(current_node.node, request.node)] ** self.alpha) / (
                                    self.network.links[(current_node.node, request.node)].distance)
                        probability.append((request, total))
                    else:
                        total += (self.pheromone[(current_node.node, request.node)] ** self.alpha) / (
                                    self.network.links[(current_node.node, request.node)].distance
                                    + request.end - (
                                                self.network.links[(current_node.node, request.node)].distance + solution_time))
                        probability.append((request, total))
...
if __name__ == "__main__":
    seed = 1
    np.random.seed(seed)
    problem1 = ProblemTD("data/dvrptw/1000/h1000C1_10_3.csv")
